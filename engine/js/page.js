...
PBS.KIDS.storybook.page = function (GLOBAL, PBS, config, pageNum, options) {
	...
	that.init = function () {
		...
		var spec, i, curTextArea, curSprite, curCycler, curDrawingPad, curSuggestedBooks, pageSoundPersistence;
		...
		if (config && config.content) {
			for (i = 0; i < config.content.length; i += 1) {
				...
				case "SuggestedBooks":
					...
					config.content[i].parentWidth = width;
					config.content[i].parentHeight = height;
					config.content[i].className = "pbsPage" + pageNum + "SuggestedBooks" + (cyclerArray.length + 1);
							
					curSuggestedBooks = sb.suggestedBooks(GLOBAL, PBS, config.content[i]);
					curSuggestedBooks = sb.makeInteractionObject(GLOBAL, PBS, curSuggestedBooks);		
					
					// If a sound is specified
					if (config.content[i].sound) {
						curSuggestedBooks = sb.makeAudible(GLOBAL, PBS, audioPlayer, curSuggestedBooks, config.content[i].sound);
					}			
