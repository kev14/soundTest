...
PBS.KIDS.storybook.book = function (GLOBAL, PBS, storybookContainerElement, config) {
	...
	/*	updatePosition = function () {
		...
		onNavigateComplete = function () {
			...
			if (targetPageIndex !== curPageIndex) {
				...
				if (targetPageIndex > curPageIndex) {
					...
					pagesContainerElement.style.width = "0%";
			...
			if (targetPageIndex === -1) {
				...
				if (curOrientation === "SINGLE-PAGE") {
					...
					if (leftPageIndex === curPageIndex) {
						if (pages[leftPageIndex].pageSound && speechState === true) {
							// Play the page sound
							audioPlayer.play(pages[leftPageIndex].pageSound);
						}
					// If the page is the right page
					} else {
						if (pages[rightPageIndex].pageSound && speechState === true) {
							// Play the page sound
							audioPlayer.play(pages[rightPageIndex].pageSound);
						}
					}
				// if two-page layout
					...
					} else if (pages[rightPageIndex].pageSound && speechState === true) {
						// Play the page sound
						loud();
						audioPlayer.play(pages[leftPageIndex].pageSound);
						audioPlayer.addEventListener("PLAY_COMPLETE", rightPageSoundComplete);
			...
			if (targetPageIndex === -1) {
				...
				if (readButtonElement){
					readButtonElement.style.display = "block";					
				}
				if (readMyselfButtonElement){
					readMyselfButtonElement.style.display = "block";					
				}
			} else {
				// Hide page navigation buttons when at the beginning and end
				if (curOrientation === "SINGLE-PAGE") {
					if(readButtonElement){
						readButtonElement.style.display = (targetPageIndex === -1) ? "block" : "none";						
					}
					if (readMyselfButtonElement){
						readMyselfButtonElement.style.display = (targetPageIndex === -1)? "block" : "none";					
				  }
					if (prevPageButtonElement) {
		...
		navigateToPageIndex = function (pageIndex, doNotAnimate) {
			...
			if (readButtonElement) {
				readButtonElement.style.display = "none";
			}
			
			if (readMyselfButtonElement) {
				readMyselfButtonElement.style.display = "none";
			}
		...
		start = function () {
			...
			//Draw readButton and add event Listener
			if(readButtonSprite){
				readButtonSprite.update();
				readButtonSprite.render();
				readButtonSprite.addEventListener("PRESS",that.read);					
			}
			
			//Draw readMyselfButton and add event Listener
			if(readMyselfButtonSprite){
				readMyselfButtonSprite.update();
				readMyselfButtonSprite.render();
				readMyselfButtonSprite.addEventListener("PRESS",that.readMyself);					
			}
			...
			if (audioPlayer) {
				if (audioLoadInitiated) {
					if (audioLoadedEnough) {
						loadingContainerElement.innerHTML = "<p id=loadingText>Loading...</p>";
					}
				} else {
						loadingContainerElement.innerHTML = "<p id=loadingText>Loading...</p>";
				}
		...
		console.log('init');
			...
			if (!initialized) {
		  	...
		  	if (bookConfig.readButton) {
					readButtonElement = GLOBAL.document.createElement("div");
		            readButtonElement.id = "pbsReadButton";
					readButtonElement.className = "pbsPagesContainer";
					
					// Create read button sprite
					bookConfig.readButton.parentElement = readButtonElement;
					bookConfig.readButton.resource = resourceLoader.addToQueue(bookConfig.readButton.url);
					readButtonSprite = sb.sprite(GLOBAL, PBS, bookConfig.readButton);
					var btnState = true;
					readButtonSprite = sb.makeInteractionObject(GLOBAL, PBS, readButtonSprite, btnState);
					pagesContainerElement.appendChild(readButtonElement);
				}
				
				if (bookConfig.readMyselfButton) {
					readMyselfButtonElement = GLOBAL.document.createElement("div");
		           readMyselfButtonElement.id = "pbsReadMyselfButton";
					readMyselfButtonElement.className = "pbsPagesContainer";
					
					// Create readmyself button sprite
					bookConfig.readMyselfButton.parentElement = readMyselfButtonElement;
					bookConfig.readMyselfButton.resource = resourceLoader.addToQueue(bookConfig.readMyselfButton.url);
					readMyselfButtonSprite = sb.sprite(GLOBAL, PBS, bookConfig.readMyselfButton);
					readMyselfButtonSprite = sb.makeInteractionObject(GLOBAL, PBS,readMyselfButtonSprite, btnState);
					pagesContainerElement.appendChild(readMyselfButtonElement);
				}
